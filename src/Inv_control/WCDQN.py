...
class WCDQN(BaseAgent):
    ...
    def get_actions_upper_bound(self, ):
        ...
        lb = self.env.max_resource/(1-self.gamma) * self.lambda_set
        Q = Q + lb[None, :]
        Q_min = np.min(Q, 1)
        return Q_min
    ...
    def replay(self, morph_cadence = 1):
        ...
        self.subproblem_model.fit(np.array(subproblem_states_conc), np.array(targets), epochs=1, use_multiprocessing=False, verbose=0)
    ...
    def get_upper_bound(self, subproblem_states, num_lambda=6):
        ...
        lb = self.env.max_resource / (1 - self.gamma)
...
if __name__ == '__main__':
    ...
    from conf_1 import config
    from MakeToStock import MakeToStock
