...
class deadlineSchedulingEnv(gym.Env):
    ...
    '''
    Deadline scheduling of EV charging problem. Gym environemnt of the subproblems.
    '''
    ...
    def _calRewardAndState(self, action):
        ...
        self.currentState = currentState = np.array([self.arm[0][0], self.arm[0][1]], dtype=np.float32)
    ...
    def step(self, action):
        ...
        # if train == True  update done to reset episode
        if self.train:
    ...
    def _newArrival(self): 
        ...
        #####
        if self.episodeTime >= self.episodeLimit:
            self.reset()
        #####
    ...
    def reset(self):
        ...
        ###
        self.currentState = np.array([self.arm[0][0], self.arm[0][1]], dtype=np.float32)
        ###
        return initialState
...
if __name__ == '__main__':
    ...
    for step in range(n_steps):
        ...
        print(f'state: {env.arm[0]} action: {x[step]} nextstate: {nextState}  reward: {reward} done: {done}')
        print("---------------------------------------------------------")
        if done:
